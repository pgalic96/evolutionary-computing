...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (KatsuuraSecondIter.txt) DO (
	...
	FOR /L %%I IN (4,1,10) DO (
		@echo progress %%H >> KatsuuraSecondOutput.txt
		java -jar testrun.jar -submission=player22 -evaluation=KatsuuraEvaluation -seed=%%I >> KatsuuraSecondOutput.txt
	)
	ECHO %%H
) 