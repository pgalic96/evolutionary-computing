...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (BentCigFirstIter.txt) DO (
	...
	FOR /L %%I IN (1,1,5) DO (
		@echo progress %%H >> BentCigarFirstOutput.txt
		java -jar testrun.jar -submission=player22 -evaluation=BentCigarFunction -seed=%%I >> BentCigarFirstOutput.txt
	)
	ECHO %%H
) 