...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (SchaffersSecondIter.txt) DO (
	...
	FOR /L %%I IN (6,1,50) DO (
		java -jar testrun.jar -submission=player22 -evaluation=SchaffersEvaluation -seed=%%I >> SchaffersSecondOutput.txt
	)
	ECHO %%H
) 