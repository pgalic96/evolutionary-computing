...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (KatsuuraFirstIter.txt) DO (
	...
	FOR /L %%I IN (1,1,3) DO (
		@echo progress %%H >> KatsuuraFirstOutput.txt
		java -jar testrun.jar -submission=player22 -evaluation=KatsuuraEvaluation -seed=%%I >> KatsuuraFirstOutput.txt
	)
	ECHO %%H
) 