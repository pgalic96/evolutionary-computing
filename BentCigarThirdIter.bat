...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (BentCigarThirdIter.txt) DO (
	...
	FOR /L %%I IN (51,1,100) DO (
		...
		@echo progress %%H >> BentCigarThirdOutput.txt
		java -jar testrun.jar -submission=player22 -evaluation=BentCigarFunction -seed=%%I >> BentCigarThirdOutput.txt
	)
	ECHO %%H
) 