...
FOR /F "tokens=1,2,3,4,5,6,7,8" %%A IN (SchafferThirdIter.txt) DO (
	...
	FOR /L %%I IN (51,1,100) DO (
		java -jar testrun.jar -submission=player22 -evaluation=SchaffersEvaluation -seed=%%I >> SchaffersThirdOutput.txt
	)
	ECHO %%H
) 